...
class _UserInfoUpdate extends State<UserInfoUpdate> {
  List<String> bankList = AppCore.getBankList() as List<String>;
    ...
    return ScreenFrame(
      body: ListView(
        children: [
          ...
          DropdownButton(
            isExpanded: true,
            value: AppCore.instance.getUser().bank,
            items: bankList.map(
              (value) { 
            ...
            onChanged: (value) {
              setState(() {
                AppCore.instance.getUser().bank = value.toString();
              });
            }
          ),
          SizedBox(
          ...
          TextButton(
            ...
            onPressed: () async {
              if (await AppCore.instance.getUser().userUpdate() as String == '1') { // 저장 성공 시
